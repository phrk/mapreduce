...
void MRInterResult::flush_wbuffer()
{
	...
	{
		throw std::string("flush_wbuffer:: cant writev"+std::string(strerror(errno)));
	}
	m_wbuffer_size = 0;
}
...
void MRInterResult::addEmit(KeyType key, EmitType *emit)
{
	if (mode!=IR_WRITING)
	{
		throw std::string("MRInterResult::addEmit error mode!=IR_WRITING");
	}
	std::string dump;
...
EmitType *MRInterResult::restore(off_t offset)
{
	...
	{
		throw std::string("MRInterResult::preload: READ ERROR\n");
	}
...
void MRInterResult::preload(uint64_t key, bool cid)
{
	if (mode!=IR_READING)
	{
		throw std::string("MRInterResult::preload error mode!=IR_READING");
	}
...
EmitType* MRInterResult::getEmit(uint64_t key, bool cid)
{
	if (mode!=IR_READING)
	{
		throw std::string("MRInterResult::getEmit error mode!=IR_READING");
	}
	if (cid==false)
	{
		hiaux::hashtable<uint64_t, EmitType*>::iterator cache_it = m_emit_cache0.find(key);
	...
	{
		hiaux::hashtable<uint64_t, EmitType*>::iterator cache_it = m_emit_cache1.find(key);
