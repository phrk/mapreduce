	...
	m_filename(filename)
{
	...
	if (m_fd == -1)
	{
		throw std::string("InterResultLoader:: open "+std::string(strerror(errno)));
	}
	
	m_len = lseek(m_fd, 0, SEEK_END);
	if (m_len == -1)
	{
		throw std::string("InterResultLoader:: lseek "+std::string(strerror(errno)));
	}
	...
	if ((void*)p == (void*)-1)
	{
		throw std::string("InterResultLoader:: mmap "+std::string(strerror(errno)));
	}
}
...
InterResultLoader::~InterResultLoader()
{
	if (!closed)
	{
		if (munmap(p, m_len) == -1)
		{
			throw std::string("InterResultLoader::~InterResultLoader:: munmap "
					+std::string(strerror(errno)));
		}
		if (close(m_fd) == -1)
		{
			throw std::string("InterResultLoader::~InterResultLoader:: close "
					+std::string(strerror(errno)));
		}
	}
}
...
void InterResultLoader::deleteFile()
{
	if (munmap(p, m_len) == -1)
	{
		throw std::string("InterResultLoader::deleteFile:: munmap "+std::string(strerror(errno)));
	}
	
	if (close(m_fd) == -1)
	{
		throw std::string("InterResultLoader::deleteFile:: close "
				+std::string(strerror(errno)));
	}
	
	if (unlink(m_filename.c_str()) == -1)
	{
		throw std::string("InterResultLoader::deleteFile:: unlink "+std::string(strerror(errno)));
	}
