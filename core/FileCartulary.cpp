...
{
	hiaux::hashtable<int, int>::iterator it = filecache.find(fileid);
	if (it != filecache.end())
	{
		return it->second;
	}
...
{
	hiaux::hashtable<int, int>::iterator it = filecache.begin();
...
{
	hiaux::hashtable<int, int>::iterator it = filecache.find(id);
	if (it != filecache.end())
	{
		return it->second;
	}
